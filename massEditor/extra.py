...
def minitechSortingAlgorithm(q):
    ...
    from .models import ListOfObjects
    from .logic import search
    from .store import state
    
    names, depths = state.names, state.depths
...
# foods = foods.sort(key=lambda o: (depths[int(o.id)] if int(o.id) in depths.keys() else 9999, getTotalFoodValue(int(o.id))))

# for id, o in foods.items():
#     print(f"{depths[id] if id in depths.keys() else 9999:<6} {o.foodValue:<6} {id:<6} {o.name}")
...
# from massEditor.util import list_dir
# from massEditor.config import OUTPUT_PATH

# existing_sounds = list_dir(OUTPUT_PATH / "sounds", file=True)
# existing_sounds = LO([e.replace(".aiff", "") for e in existing_sounds if ".aiff" in e])
...
#             used_sounds.append(s)
            
# unused_sounds = list(existing_sounds - used_sounds)
